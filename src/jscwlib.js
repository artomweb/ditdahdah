...
export class jscw {
  constructor(params) {
    ...
    var play_svg =
      "data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxMCIgaGVpZ2h0PSIxMCIgdmlld0JveD0iMCAwIDEwIDEwIj48cGF0aCBkPSJNMyAyIEwzIDggTDcgNSBaIiBzdHlsZT0iZmlsbDojZmZmO3N0cm9rZS13aWR0aDowLjM7c3Ryb2tlOiMwMDA7c3Ryb2tlLWxpbmVqb2luOmJldmVsOyIvPjwvc3ZnPgo=";
    var pause_svg =
    ...
    var stop_svg =
      "data:image/svg+xml;base64,";
    ...
    var alphabet = {
      ...
      I: "..",
      i: "..",
      ı: "..", // dotless i (see https://en.wikipedia.org/wiki/Dotless_I)
      J: ".---",
      j: ".---",
      K: "-.-",
      k: "-.-",
      ...
      S: "...",
      s: "...",
      ſ: "...", // long s (see https://en.wikipedia.org/wiki/Long_s)
      T: "-",
      t: "-",
      U: "..-",
      u: "..-",
    ...
    setTimeout(() => {
      ...
      if (lamp) {
        that.onLampOff = function () {
          lamp.style.backgroundColor = "white";
        };
        that.onLampOn = function () {
    ...
    try {
      ...
      this.mode = "embed";
      console.log(
        "AudioContext not supported. Fall back to HTML audio element"
      );
    }
    ...
    this.init = function () {
      if (this.mode == "embed") {
        ...
        this.noiseFilterL.frequency.setValueAtTime(
          400,
        ...
        this.noiseFilterH.frequency.setValueAtTime(
          700,
          this.audioCtx.currentTime
        );
        ...
        var noiseBuffer = this.audioCtx.createBuffer(
          1,
          bufferSize,
    ...
    this.setQ = function (q) {
      ...
      this.gainNodeLimiter.gain.setValueAtTime(
        1.8 * Math.exp(-0.115 * q),
        this.audioCtx.currentTime
      );
      this.updateControls();
    };
    ...
    this.updateControls = function () {
      ...
      if (this.control_labels["vol"]) {
        this.control_labels["vol"].innerHTML =
          Math.round(this.playvolume * 100) + "&nbsp;%";
      }
    ...
    this.enableControls = function (obj, b) {
      ...
      for (var p in obj.control_inputs) {
        if (p == "vol" || p == "edge" || p == "freq") continue;
      ...
      for (var i = 0; i < this.playTiming.length - 1; i++) {
        if (this.playTiming[i]["v"] > 0) {
          ctx.fillRect(
            this.playTiming[i]["t"],
            10,
            this.playTiming[i + 1]["t"] - this.playTiming[i]["t"],
            5
          );
        }
      }
    ...
    this.refresh_download_link = function () {
      if (this.btn_down) {
        ...
        if (this.btn_down.href.length > 8000) {
          console.log(
            "Warning: Download URL truncated, requested URL was too long."
          );
    ...
    this.setTimers = function () {
      ...
      if (this.onFinished) {
        this.timers.push(
          setTimeout(this.onFinished, this.getRemaining() * 1000)
        );
      }
    }; // setTimers
    ...
    this.gen_morse_timing = function (c, time) {
      ...
      if (!l) {
        console.log(
          "Don't know CW for character: '" + c + "', replacing with space."
        );
        l = " ";
      }
      ...
      for (var j = 0; j < l.length; j++) {
        ...
        if (el != " ") out.push({ t: time, v: this.volume });
    ...
    this.progressbarUpdate = function (obj) {
      if (obj.progressbar) {
        ...
        obj.progresslabel.innerHTML =
          obj.fmtTime(sec, sign) +
          " /" +
          obj.fmtTime(obj.getLength() - obj.textStart, "");
